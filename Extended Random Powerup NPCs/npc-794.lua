...
local customPowerupAvailable = false

local function getRandomID()
	local id = RNG.randomEntry(NPC.POWERUP)
	while (id < 751 and customPowerupAvailable) or invalidNPCs[id] do
...
function randomizer.onInitAPI()
	...
	registerEvent(randomizer, "onStart")
	registerEvent(randomizer,"onTick")
	registerEvent(randomizer, "onBlockHit")
end

function randomizer.onStart()
	for i, v in ipairs(NPC.POWERUP) do
		if v >= 751 then
			customPowerupAvailable = true
			return
		end
	end
	Misc.warn("No custom powerup IDs available for NPC-".. npcID .. " to transform into.")
end
