...
catSuit.settings = { 
	...
	canStatue = true, --Can the player become a statue? (true by default)
	spawnStatueCoins = true --Does the player spawn coins when in statue form? (true by default)
}
...
function catSuit.onInitPowerupLib()
	...
	catSuit.spritesheets = {
		catSuit:registerAsset(CHARACTER_MARIO, "mario-lucky_catsuit.png"),
		catSuit:registerAsset(CHARACTER_LUIGI, "luigi-lucky_catsuit.png"),
	...
	catSuit.iniFiles = {
		catSuit:registerAsset(CHARACTER_MARIO, "mario-lucky_catsuit.ini"),
		catSuit:registerAsset(CHARACTER_LUIGI, "luigi-lucky_catsuit.ini"),
...
function catSuit.onTickPowerup(p) 
    ...
    if aw then aw.preventWallSlide(p) end -- prevents the anotherwalljump from running when wall-running
	...
	------------ STATUE HANDLING ------------
	if data.state == STATE_STATUE then
		data.statueTimer = data.statueTimer + 1
		p.speedX = 0
		p.keys.left = KEYS_UP
		p.keys.right = KEYS_UP
		p.keys.down = KEYS_UP
		if (p.keys.jump or p.keys.altJump) and p:isOnGround() then data.statueTimer = 400 end
		if p.keys.altRun == KEYS_UP then
			data.statueHeldTimer = data.statueHeldTimer + 1
		else
			data.statueHeldTimer = 0
		end
		
		for _, npc in NPC.iterateIntersecting(p.x, p.y, p.x + p.width, p.y + p.height * 1.5) do
			if (not npc.friendly) and npc.despawnTimer > 0 and (not npc.isGenerator) and npc.forcedState == 0 and npc.heldIndex == 0 and not data.hittedNPCs[npc.idx] and not p:isOnGround() then
				npc:harm()
			end
		end
		...
		--gives coins when falling. Remove the brackets if you want this
		--code stolen by MDA's Coin Box

		data.timer = (data.timer or 0) + math.abs(p.speedY)

		if data.timer > 48 and settings.spawnStatueCoins then
			data.timer = 0
			addCoins(1)
