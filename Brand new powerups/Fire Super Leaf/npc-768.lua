...
local powerup = require("powerups/fireLeaf")

local powerup = {}
local npcID = NPC_ID

local cp = require("customPowerups")
local fireLeaf = cp.addPowerup("Fire Leaf", "powerups/fireLeaf", npcID)
...
function powerup.onTickNPC(v)
	...
	if lunatime.tick() % 8 == 0 then
		local e = Effect.spawn(12, RNG.randomInt(v.x, v.x + 32), RNG.randomInt(v.y, v.y - 32))
		e.speedY = -3
	end
