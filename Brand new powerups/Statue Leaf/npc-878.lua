...
function powerup.onInitAPI()
	...
	registerEvent(powerup, "onNPCCollect")
	Cheats.register("needastatueleaf",{
		isCheat = true,
		activateSFX = 12,
		aliases = {"needastoneleaf"},
		onActivate = (function() 
...
function powerup.onNPCCollect(eventObj, v, player)
	if npcID ~= v.id or v.isGenerator then return end

	z = NPC.spawn(169, player.x, player.y)
	z.animationFrame = -999
	z.animationTimer = 999
	z.speedX = player.speedX
	z.speedY = player.speedY
end
