...
catSuit.cheats = {"needaninvincibilitybell","needaninvincibilitycatsuit","needainvincibilitybell","needainvincibilitycatsuit"}
catSuit.settings = { 
...
function catSuit.onInitPowerupLib()
	...
	catSuit.spritesheets = {
		catSuit:registerAsset(CHARACTER_MARIO, "mario-Icatsuit.png"),
		catSuit:registerAsset(CHARACTER_LUIGI, "luigi-Icatsuit.png"),
	...
	catSuit.iniFiles = {
		catSuit:registerAsset(CHARACTER_MARIO, "mario-Icatsuit.ini"),
		catSuit:registerAsset(CHARACTER_LUIGI, "luigi-Icatsuit.ini"),
...
local function handleSwiping(p,left,right,top,bottom,hitboxWidth,canCombo) -- handles breaking blocks & harming npcs when doing specific actions in the cat suit
	...
	-- If the block should be broken, destroy it
		if not block.isHidden and not data.hittedBlocks[block.idx] and not Block.LAVA_MAP[block.id] and not Block.PLAYER_MAP[block.id] and not Block.SEMISOLID_MAP[block.id] then
...
catSuit.ignore = {};
catSuit.ignore[108] = true;

local function starmanFilter(v)
	return Colliders.FILTER_COL_NPC_DEF(v) and not catSuit.ignore[v.id];
end
...
function catSuit.onTickPowerup(p) 
	...
	-- if the player's busy being something else, bring the player's state back to normal
	...
	if data.state == STATE_SWIPE then
		...
		if data.swipeTimer >= 20 then
			...
			if settings.canDive and (p.keys.run == KEYS_DOWN or p.keys.altRun == KEYS_DOWN) and not isOnGround(p) and not p:mem(0x36,FIELD_BOOL) and data.canDive and not canWallClimb(p) then
