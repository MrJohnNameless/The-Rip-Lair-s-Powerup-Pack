...
local DKG = require("powerups/DKG")
local mushroom = {}
local npcID = NPC_ID

local cp = require("customPowerups")
local DKG = cp.addPowerup("Donkey Goomba", "powerups/DKG", npcID)
...
function mushroom.onInitAPI()
	...
	Cheats.register("needadonkeygoomba",{
		isCheat = true,
		activateSFX = 12,
		aliases = DKG.aliases,
		onActivate = (function() 
...
function mushroom.onPostNPCCollect(v,p)
	...
	if not p.data.DKG then return end
