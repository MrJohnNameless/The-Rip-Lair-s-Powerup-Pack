...
function lightning.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.timer = 0
		data.timer2 = 0
		data.speed = NPC.config[v.id].speed
	end
	
	data.timer = data.timer + 1
	data.timer2 = data.timer2 + 1
	
	if data.timer2 >= NPC.config[v.id].lifetime then v:kill(9) end
	...
	local list = Colliders.getColliding{a = v, b = tbl, btype = Colliders.BLOCK, filter = function(other)
		...
		if NPC.config[v.id].laserBounce and (Block.config[other.id].floorslope ~= 0 or Block.config[other.id].ceilingslope ~= 0) then
			...
			if Block.config[other.id].floorslope ~= 0 then
				if data.rotation % 180 == 90 then
					v.speedX = 0
					v.speedY = -data.speed
				else
					v.speedX = data.speed * Block.config[other.id].floorslope
					v.speedY = 0
				end
			end
			
			if Block.config[other.id].ceilingslope ~= 0 then
				if data.rotation % 180 == 90 then
					v.speedX = 0
					v.speedY = data.speed
				else
					v.speedX = -data.speed * Block.config[other.id].ceilingslope
					v.speedY = 0
				end
			end
	...
	if data.rotation % 90 == 0 then
		v.width = NPC.config[v.id].width
		v.height = NPC.config[v.id].height
	else
		v.width = NPC.config[v.id].height
		v.height = NPC.config[v.id].width
	end
end
