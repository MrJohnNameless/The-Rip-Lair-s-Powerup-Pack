...
apt.iniFiles = {
	Misc.resolveFile("mario-ap_pacorn.ini"),
	Misc.resolveFile("luigi-ap_pacorn.ini"),
}
...
apt.items = {972} -- Items that can be collected.
...
function apt.onEnable(p)
	...
	apt.spritesheets[p.character] = Graphics.loadImage(Misc.resolveFile(characterList[p.character].."-ap_pacorn.png"))
end
...
function apt.onTickPowerup(p)
	...
	if table.ifind(SMM2Costumes, p:getCostume()) then
		if lunatime.tick() == 1 or p:getCostume() ~= previousCostume then
			if altFrames then
				apt.spritesheets[p.character] = Graphics.loadImage(Misc.resolveFile(p:getCostume().."-ap_pacornb.png"))
			else
				apt.spritesheets[p.character] = Graphics.loadImage(Misc.resolveFile(p:getCostume().."-ap_pacorn.png"))
			end
		end
...
function apt.onTickEndPowerup(p) --This part of the code is the messiest, since it's what I did first. Definitely coming back so that I can clean it up a bit later on.
	...
	boostTimer = boostTimer - 1
		...
		if lunatime.tick() % 4 == 0 then
			Effect.spawn(972, RNG.randomInt(p.x, p.x + 32), RNG.randomInt(p.y, p.y + 32), RNG.randomInt(1,4))
		end
...
function apt.onExit()
	if activePlayer and cp.getCurrentName(activePlayer) == "P-Acorn" then cp.setPowerup("Super Acorn", activePlayer, true) end
end
...
function apt.onPlayerKill(o,p)
	...
	if table.ifind(SMM2Costumes, player:getCostume()) and player:mem(0x46, FIELD_WORD) == 800 then
		Graphics.sprites.effect[deathEffect].img = Graphics.loadImage(Misc.resolveFile(player:getCostume().."-ap_pacornmiss.png"))
	end
end
