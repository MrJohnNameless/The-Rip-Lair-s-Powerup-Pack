--THANKS HOELOE FOR HELPING ME WITH EXPLOSION COLLISION STUFF!--
...
function sampleBlock.onInitAPI()
	...
	registerEvent(sampleBlock, "onPostExplosion")
end
...
function sampleBlock.onPostExplosion(e, plyr)
    if e.id >= 2 then -- there's also e.id, which is the explosion type
        for _,v in ipairs(Colliders.getColliding{a = e.collider, b = Block.SOLID, btype = Colliders.BLOCK}) do
			if v.id == blockID then
				v:remove()
				SFX.play(3)
				SFX.play(4)
				Animation.spawn(100, v.x + (v.width/2), v.y + (v.height/2))
			end
        end
    end
end
