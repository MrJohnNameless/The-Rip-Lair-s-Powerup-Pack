...
function block:onTickEndBlock()
	if blockutils.hiddenFilter(self) then
		for _,p in ipairs(Player.getIntersecting(self.x - 8, self.y - 8, self.x + self.width + 8, self.y + self.height + 8)) do
			if cp.getCurrentName(p) == "Penguin Suit" and not p.keys.down and not p.keys.left and not p.keys.right and not p.keys.run and p.speedX >= 4 then
				killBlock(self)
			end
		end
	end
